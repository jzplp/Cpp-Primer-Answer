...
TextQuery::TextQuery(std::ifstream & is) noexcept : file(new std::vector<std::string>)
{
	std::string text;
...
std::set<TextQuery::line_no>::iterator TextQuery::QueryResult::begin()
{
	return lines->begin();
}

std::set<TextQuery::line_no>::iterator TextQuery::QueryResult::end()
{
	return lines->end();
}

std::shared_ptr<std::vector<std::string>> TextQuery::QueryResult::get_file()
{
	return file;
}
